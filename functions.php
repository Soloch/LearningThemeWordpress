...
function csm_enqueue_style(){
    // -- Normalize CSS 
    wp_enqueue_style('csmnormalise', get_template_directory_uri().'/assets/css/normalize.css' , false);
...
function register_csm_menu(){
    register_nav_menu('csm-menu', 'Header Navigation Menu');
...
register_default_headers( array(
    'bandeauDuHaut' => array(
        ...
        'description' => __('Proposition1', 'isen')
...
//Créeation d'un CUSTOM POST TYPE
...
add_action( 'widget_init', 'csm_widgets_init');
function csm_widgets_init(){
    ...
    register_sidebar( array(
        ...
        'before-widget' => '<div id="%1$s" class="newsletter %2$s"',
        'after-widget' => '</div>',
        'before_title' => '<h3>',
        'after_title' => '</h3>'
    ) );
    register_sidebar( array(
        ...
        'id' => 'csm-footer-3',
        'description' => 'Widget pour le placement de la Newsletter',
        'before-widget' => '<div id="%1$s" class="contact %2$s"',
        'after-widget' => '</div>',
        'before_title' => '<h3>',
        'after_title' => '</h3>'
    ) );}